...
Causal ablation reveals opposite mechanistic regimes across scales: high-binding heads are necessary at 160M (ablation impairs recognition by $-16.7$~pp) but \emph{functionally superseded} at 2.8B (ablation improves recognition by $+33.3$~pp), providing direct evidence for the decoupling phenomenon (C5).
...
Our contributions are organized around four empirical claims (C1, C3--C5); a fifth claim concerning representational stability to prompt perturbations (C2) remains for future work (see Section~5.4):
...
\begin{enumerate}
  ...
  \item \textbf{Cross-scale causal regimes (C5).} Ablating high-binding heads reveals opposite effects: necessary at 160M but \emph{interfering} at 2.8B, providing mechanistic evidence for decoupling (Section~4.4).
\end{enumerate}
...
\hline
\end{tabular}
\caption{Correlation between EB$^*$ and behavioral accuracy across model scales.}
\end{table}
...
The 1B model occupies a transitional regime between small models (where binding directly supports behavior) and large models (where binding saturates at high levels and behavior develops through distributed or redundant representations).
...
\caption{Decoupling at 1B scale: EB$^*$ saturates early while behavioral score continues improving through step 143K.}
...
\hline
\end{tabular}
\caption{Few-shot generation scores: one-shot prompting unlocks latent knowledge when EB$^*>0.6$. All scores are generation-only (keyword rubric).}
\end{table}
...
One-shot outputs often reproduce phrasing from the provided example, inflating generation scores.
...
\paragraph{Pythia-160M: coupled regime.}
...
\paragraph{Pythia-2.8B: functionally superseded regime.}
...
\begin{itemize}
  ...
  High-binding heads---concentrated in early layers---may implement rigid attention patterns that override more flexible representations in later layers.
  Ablating them removes this interference, improving performance.
  These heads likely serve a scaffolding role during earlier training, helping the model bind multi-token terms before more flexible distributed representations develop. Their persistence at convergence reflects gradient descent's inability to prune vestigial structures rather than ongoing functional utility.
\end{itemize}
...
This ceiling convergence is consistent with activation failures rather than missing knowledge.

\paragraph{Why early-layer binding interferes at scale.}
In deep transformers, early layers often encode local and syntactic features while later layers develop semantic and task-relevant representations.
...
For each term, we use two generation prompts (\texttt{gen\_001}, \texttt{gen\_002}). In the few-shot condition, the JSONL replaces the template by prepending a short, term-specific one-shot prefix.
...
\hline
\end{tabular}
\caption{Few-shot (one-shot-prefixed) generation prompt templates.}
\end{table}
...
\centering
\includegraphics[width=\textwidth]{attention-binding-a11y-main/figures/figure4_1b_decoupling.pdf}
\caption{Decoupling at 1B scale: EB$^*$ saturates early while behavioral performance continues improving.}
\end{figure}
...
\[
\mathrm{EB}(T,\ell)
= \max_h \mathrm{BSI}(T,\ell,h) - \frac{1}{H}\sum_{h=0}^{H-1}\mathrm{BSI}(T,\ell,h).
\]
